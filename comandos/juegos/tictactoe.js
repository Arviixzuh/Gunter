...
module.exports = {
	...
	async execute(client, message, args, reaction) {
		...
		if (ttt0.id === ttt1.id) return message.channel.send({
			embed: {
				color: 15158332,
		...
		msgsiono.awaitReactions(filter, { max: 1, time: 120000 })
			.then(async collection => {
				...
				if (!reaction) {
					msgsiono.edit(limite) && msgsiono.reactions.removeAll().catch(error => { return; });
				} else {
					if (reaction.emoji.id === '909528174063415307') {
						msgsiono.edit(noquiero)
						msgsiono.reactions.removeAll().catch(error => { return; });
					}
						...
						msgsiono.edit(waiting).then(async msg => {
							...
							arribaizquierda.on("collect", async function (r) {
								...
								if (turn === 0) {
									...
									if (r.users.cache.sort(m => m.bot === r.users.remove(ttt1)).map(p => p.id).includes(ttt1.id)) return message.channel.send({
										embed: {
											color: 15158332,
									...
									if (r.users.cache.sort(m => m.bot === r.users.remove(ttt0)).map(p => p.id).includes(ttt0.id)) return message.channel.send({
										embed: {
											color: 15158332,
