...
module.exports = {
    ...
    let number = args[0]
        if (isNaN(number)) return message.channel.send({embed: {
      color: 15158332,
    ...
    number = parseInt(number)
        if (number >= 100) {
          ...
          message.channel.bulkDelete(99).catch(err => {
            message.channel.send("Ocurrio algo inesperado." + err + "xd" )
    
         if(err) return;
            
      message.channel.send({embed: {
      color: 3066993,
      description: `<a:check:907350759782350918> • ¡Se eliminaron **99** mensajes!`
    }}).then((m) => m.delete({ timeout: 10000 }))
            }) 
        } 
